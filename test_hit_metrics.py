...
from src.moola.data.pointer_transforms import start_end_to_center_length, center_length_to_start_end
...
def test_pointer_transforms():
    ...
    print(f"Start: {start.item()}, End: {end.item()} → Center: {center.item():.4f}, Length: {length.item():.4f}")
    ...
    assert abs(recovered_start.item() - start.item()) < 0.5, f"Start recovery failed: {recovered_start} vs {start}"
    assert abs(recovered_end.item() - end.item()) < 0.5, f"End recovery failed: {recovered_end} vs {end}"
    ...
    center, length = start_end_to_center_length(start, end, seq_len=104)
    print(f"Min span - Start: {start.item()}, End: {end.item()} → Center: {center.item():.4f}, Length: {length.item():.4f}")
    ...
    center, length = start_end_to_center_length(start, end, seq_len=104)
    print(f"Max span - Start: {start.item()}, End: {end.item()} → Center: {center.item():.4f}, Length: {length.item():.4f}")
...
def test_uncertainty_bias():
    ...
    print(f"Initial balance ratio (ptr/type): {loss_fn.get_task_balance_ratio():.4f} (expected: >1.0)")
    
    # Verify pointer is favored (ratio > 1.0)
    assert loss_fn.get_task_balance_ratio() > 1.0, "Pointer task should be favored"
    assert abs(uncertainties['sigma_ptr'] - 0.74) < 0.05, f"Pointer sigma should be ~0.74, got {uncertainties['sigma_ptr']}"
    assert abs(uncertainties['sigma_type'] - 1.00) < 0.1, f"Type sigma should be ~1.00, got {uncertainties['sigma_type']}"
