...
def apply_light_augmentation(batch: dict, sigma: float = 0.01) -> dict:
    ...
    return {"X": X_aug, "target": target, "mask": batch["mask"]}
...
def main():
    ...
    parser.add_argument(
        "--jitter-sigma", type=float, default=0.01, help="Jitter augmentation sigma"
    )
    parser.add_argument("--patience", type=int, default=15, help="Early stopping patience")
    parser.add_argument(
        "--output-dir", type=str, default="artifacts/encoders/pretrained", help="Output directory"
    )
    ...
    if not args.no_wandb:
        wandb.init(
            project="moola-pretrain", config=vars(args), name=f"mae_pretrain_ohlc_{args.epochs}ep"
        )
    ...
    torch.save(
        {
            ...
            "config": {"input_dim": 4, "hidden_dim": 128, "num_layers": 2, "dropout": args.dropout},
