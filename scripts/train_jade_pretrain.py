...
class JadeTrainer:
    ...
    def setup_model_and_optimizer(
        self,
    ) -> tuple[JadePretrainer, torch.optim.Optimizer, CosineWarmupScheduler, Optional[GradScaler]]:
        ...
        scaler = (
            GradScaler() if (self.config.mixed_precision and torch.cuda.is_available()) else None
        )
    ...
    def train_epoch(
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if batch_idx == 0 and torch.cuda.is_available():
                if isinstance(batch, (list, tuple)):
                    print(
                        f"Batch devices: X={batch[0].device}, mask={batch[1].device}, valid={batch[2].device}"
                    )
                else:
            ...
            # Track metrics
            metrics.update(
                {"batch": batch_idx, "epoch": epoch, "lr": optimizer.param_groups[0]["lr"]}
            )
            ...
            if torch.cuda.is_available() and batch_idx % 10 == 0:
                ...
                print(
                    f"  Batch {batch_idx}: Loss {loss.item():.6f}, GPU Memory {gpu_memory:.1f} MB"
                )
    ...
    def validate_epoch(self, model: JadePretrainer, val_loader, epoch: int) -> dict:
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(val_loader):
                ...
                metrics.update({"batch": batch_idx, "epoch": epoch})
                epoch_metrics.append(metrics)
    ...
    def _validate_features(self, df: pd.DataFrame):
        ...
        from moola.features.relativity import (
            RelativityConfig,
        ...
        for feat_name, feat_idx in zip(sparse_features, sparse_indices):
            ...
            print(
                f"  {feat_name:20s}: non-zero={non_zero_pct:.1%}, mean={mean_val:+.4f}, std={std_val:.4f}"
            )

            # PDF requirement: >50% non-zero for sparse features
            if non_zero_pct < 0.50:
                print("    ⚠️  WARNING: Non-zero % below 50% threshold (PDF requirement)")
            else:
                print("    ✓ Passed non-zero threshold")
...
def main():
    ...
    try:
        ...
        training_config = TrainingConfig(
            epochs=args.epochs, batch_size=args.batch_size, learning_rate=args.lr, seeds=[args.seed]
        )
