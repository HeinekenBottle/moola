...
def merge_all_data() -> pd.DataFrame:
    ...
    logger.info(
        f"Duration: {(merged_df.index.max() - merged_df.index.min()).days / 365.25:.1f} years"
    )
...
def save_merged_data(df: pd.DataFrame, output_path: str):
    ...
    with open(summary_path, "w") as f:
        f.write("NQ OHLCV 1-min Continuous Dataset Summary\n")
        f.write("=" * 50 + "\n")
        f.write(f"Total records: {len(output_df):,}\n")
        f.write(f"Date range: {output_df.index.min()} to {output_df.index.max()}\n")
        f.write(
            f"Duration: {(output_df.index.max() - output_df.index.min()).days / 365.25:.1f} years\n"
        )
...
def main():
    ...
    try:
        ...
        # Save merged data
        output_path = (
            "/Users/jack/projects/moola/data/raw/nq_ohlcv_1min_2020-09_2025-10_continuous.parquet"
        )
