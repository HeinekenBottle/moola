...
def check_registry() -> tuple[bool, str]:
    ...
    try:
        ...
        if not registry_models.issubset(allowed_models):
            return (
                False,
                f"Registry contains unauthorized models: {registry_models - allowed_models}",
            )
...
def check_pointer_encoding() -> tuple[bool, str]:
    ...
    try:
        ...
        if abs(center.item() - expected_center) > 1e-6:
            return (
                False,
                f"Center conversion incorrect: got {center.item()}, expected {expected_center}",
            )

        if abs(length.item() - expected_length) > 1e-6:
            return (
                False,
                f"Length conversion incorrect: got {length.item()}, expected {expected_length}",
            )
    ...
    try:
        ...
        if splits_file.exists():
            ...
            if "purge_window" in content and "forward-chaining" in content:
                return (
                    True,
                    "✓ Temporal split implementation found with purge window and forward-chaining",
                )
            else:
                return (
                    False,
                    "Temporal split file missing purge window or forward-chaining implementation",
                )
        else:
...
def check_pretraining_config() -> tuple[bool, str]:
    ...
    try:
        ...
        return (
            True,
            "✓ Pretraining uses paper-strict settings (batch 64, epochs 100, dropout 0.15, jitter 0.01, mask 0.4)",
        )
    except Exception as e:
...
def check_supervised_config() -> tuple[bool, str]:
    ...
    try:
        ...
        train_config_path = (
            Path(__file__).parent.parent / "src" / "moola" / "configs" / "train" / "multitask.yaml"
        )
        ...
        if train_cfg.get("data", {}).get("batch_size") != 29:
            return (
                False,
                f"Training batch size: {train_cfg.get('data', {}).get('batch_size')}, expected 29",
            )
        ...
        for model_name, config_path in model_configs.items():
            ...
            if arch.get("input_dropout") != 0.25:
                return (
                    False,
                    f"{model_name} input_dropout: {arch.get('input_dropout')}, expected 0.25",
                )

            if arch.get("recurrent_dropout") != 0.65:
                return (
                    False,
                    f"{model_name} recurrent_dropout: {arch.get('recurrent_dropout')}, expected 0.65",
                )

            if arch.get("dense_dropout") != 0.5:
                return (
                    False,
                    f"{model_name} dense_dropout: {arch.get('dense_dropout')}, expected 0.5",
                )

        return (
            True,
            "✓ Supervised training uses paper-strict settings (batch 29, input 0.25, recurrent 0.65, dense 0.5)",
        )
    except Exception as e:
...
def check_augmentation() -> tuple[bool, str]:
    ...
    try:
        ...
        return (
            True,
            "✓ Augmentation uses paper-strict settings (jitter 0.03, magwarp 0.2, multiplier 3)",
        )
    except Exception as e:
