...
def check_data_pipeline():
    ...
    try:
        ...
        from moola.data.pretrain_pipeline import (
            PretrainPipeline,
            validate_pretrain_batch,
        )
...
def check_training_scripts():
    ...
    scripts = ["scripts/run_mae_pretrain.py", "scripts/run_supervised_train.py"]
...
def check_target_metrics():
    ...
    target_metrics = {
        "pretrain": {"val_mae_gap_min": 5.0, "val_mae_gap_max": 15.0},  # %  # %
        "supervised": {
    ...
    assert (
        0
        <= target_metrics["pretrain"]["val_mae_gap_min"]
        <= target_metrics["pretrain"]["val_mae_gap_max"]
        <= 100
    )
    ...
    print(
        f"   Pretrain val MAE gap target: {target_metrics['pretrain']['val_mae_gap_min']}-{target_metrics['pretrain']['val_mae_gap_max']}%"
    )
    print(
        f"   Supervised targets: Hit@±3 ≥{target_metrics['supervised']['hit_at_3_min']:.0%}, "
...
def main():
    parser = argparse.ArgumentParser(
        description="Comprehensive QA Checklist for Clean Training Pipeline"
    )
    parser.add_argument(
        "--check",
        type=str,
        choices=["rsync", "invariants", "float32", "pipeline", "scripts", "makefile", "metrics"],
    ...
    if args.check:
        ...
        if args.output:
            with open(args.output, "w") as f:
                json.dump(
                    {"timestamp": str(Path.cwd()), "all_passed": result, "checks": result},
                    f,
                    indent=2,
                )
