#!/usr/bin/env python3
"""CRF Fine-tuning on Position Encoding Model - 20 Epochs.

Loads best position encoding checkpoint (epoch 95) and fine-tunes with CRF layer
to enforce contiguous span predictions. Tracks F1 and IoU metrics.

Usage:
    python3 scripts/finetune_position_crf_20ep.py \
...
def train_epoch(model, loader, optimizer, epoch, device):
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        # Loss 3: Span detection (CRF or soft span)
        if model.use_crf and "span_emissions" in output:
            # CRF loss: convert binary targets to tags (0=out, 1=in)
            target_tags = binary.long()
            mask = torch.ones_like(target_tags, dtype=torch.bool)
            loss_span = -model.crf(
                output["span_emissions"], target_tags, mask=mask, reduction="mean"
            )
        else:
            # Fallback: soft span loss
...
def eval_epoch(model, loader, device):
    """Evaluate one epoch with CRF decoding."""
    model.eval()
    total_loss = 0
    span_f1_scores = []
    span_iou_scores = []
...
def main():
    parser = argparse.ArgumentParser(
        description="Fine-tune position encoding model with CRF (20 epochs)"
    )
    parser.add_argument(
        "--checkpoint", type=str, required=True, help="Path to position encoding checkpoint"
    )
    parser.add_argument("--data", type=str, required=True, help="Path to training data")
    parser.add_argument("--output", type=str, default="artifacts/position_crf_finetuned/")
    ...
    print("Creating JadeCompact with CRF layer enabled...")
    model = JadeCompact(
        ...
        use_crf=True,  # ← ACTIVATE CRF LAYER
    )
    ...
    print(f"\nFine-tuning for {args.epochs} epochs with CRF layer...")
    best_f1 = 0
    ...
    print(f"\n{'='*70}")
    print("✅ CRF FINE-TUNING COMPLETE")
    print(f"{'='*70}")
