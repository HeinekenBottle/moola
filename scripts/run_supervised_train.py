...
from moola.metrics.joint_metrics import compute_hit_metrics, compute_joint_metrics
from moola.models.registry import (
    build,
...
def compute_uncertainty_weighted_loss(
    ptr_loss: torch.Tensor,
    cls_loss: torch.Tensor,
    log_sigma_ptr: torch.Tensor,
...
def evaluate_model(
    model: nn.Module, val_loader: DataLoader, device: torch.device, epoch: int = 0
) -> dict:
    ...
    f1_macro = f1_score(
        cls_targets.numpy(), cls_pred_labels.numpy(), average="macro", zero_division=0
    )
    ...
    joint_metrics = compute_joint_metrics(
        ptr_preds, cls_preds, ptr_targets, cls_targets, tolerance=3
    )
...
def main():
    ...
    parser.add_argument(
        "--pretrained-encoder", type=str, default=None, help="Path to pretrained encoder"
    )
    parser.add_argument("--patience", type=int, default=20, help="Early stopping patience")
    parser.add_argument("--scheduler-patience", type=int, default=10, help="LR scheduler patience")
    parser.add_argument(
        "--output-dir", type=str, default="artifacts/models", help="Output directory"
    )
    ...
    train_loader = setup_optimized_dataloader(
        list(zip(X, y)), batch_size=args.batch_size, shuffle=True, num_workers=4  # Mock dataset
    )

    val_loader = setup_optimized_dataloader(
        list(zip(X, y)), batch_size=args.batch_size, shuffle=False, num_workers=4  # Mock dataset
    )
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_pbar:
            ...
            if use_amp:
                with autocast():
                    ...
                    log_sigma_ptr = getattr(
                        model, "log_sigma_ptr", torch.tensor(0.0, device=device)
                    )
                    log_sigma_cls = getattr(
                        model, "log_sigma_cls", torch.tensor(0.0, device=device)
                    )
            ...
            # Update progress bar
            train_pbar.set_postfix({"loss": combined_loss.item(), "type": loss_type})
        ...
        # Logging
        logger.info(
            f"Epoch {epoch+1}: Loss={avg_loss:.6f}, Hit@±3={hit_at_3:.4f}, "
    ...
    logger.info(
        f"Final Results: Hit@±3={final_metrics['hit_at_3']:.4f}, "
