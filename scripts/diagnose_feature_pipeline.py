...
def main():
    ...
    if not data_path.exists():
        ...
        alt_path = (
            Path(__file__).parent.parent / "data/raw/nq_ohlcv_1min_2020-09_2025-09_fixed.parquet"
        )
    ...
    print(
        f"Valid ratio:            {mask_stats['valid_ratio_%']:.2f}% (expected: {mask_stats['expected_valid_ratio_%']:.2f}%)"
    )
    ...
    print(
        f"Swing highs detected:   {swing_stats['n_swing_highs_detected']:,} ({swing_stats['SH_detection_rate_%']:.2f}%)"
    )
    print(
        f"Swing lows detected:    {swing_stats['n_swing_lows_detected']:,} ({swing_stats['SL_detection_rate_%']:.2f}%)"
    )

    expected_swing_rate = 20  # Expect at least 20% detection rate
    if (
        swing_stats["SH_detection_rate_%"] >= expected_swing_rate
        and swing_stats["SL_detection_rate_%"] >= expected_swing_rate
    ):
    ...
    if len(high_zero_candles) > 0:
        issues.append("⚠️  WARNING: High zero rates in candle features:")
    ...
    for _, row in dist_df_subset.iterrows():
        ...
        if row["zero_rate_%"] > 30:  # Too many zeros = swings not detected
            dist_zero_ok = False
            issues.append(
                f"⚠️  WARNING: High zero rate for {row['feature']}: {row['zero_rate_%']:.2f}%"
            )
