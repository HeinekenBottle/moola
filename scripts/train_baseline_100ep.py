...
def create_expansion_labels(expansion_start, expansion_end, window_length=105):
    ...
    CHANGED: Normalized countdown to [0, 1] range for gradient balance.
    Before: countdown in [-20, 20] → Huber loss ~10.08 (91% of total)
    After:  countdown in [0, 1]    → Huber loss ~0.5 (comparable to other tasks)
    """
    ...
    countdown = -countdown
    # Normalize to [0, 1] instead of clipping to [-20, 20]
