...
def precompute_features(data_path: str, output_dir: str, config: PrecomputeConfig) -> dict:
    ...
    X_full, valid_mask_full, feature_meta = build_relativity_features(
        df[["open", "high", "low", "close"]], relativity_cfg.dict()
    )
    ...
    print("  2. Update training script to use precomputed features")
    print("  3. Run training with fast loader (5s load vs 1-3h compute)")
    ...
    try:
        ...
        if args.config:
            ...
            config = PrecomputeConfig(
                window_length=args.window_length, warmup_bars=args.warmup_bars
            )
