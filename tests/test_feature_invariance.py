...
class TestRelativityInvariance:
    ...
    def test_feature_bounds(self, sample_data):
        ...
        print("Feature bounds validated:")
        print("  Shape features in [0,1] or [-1,1]")
        print("  Distance features in [-3,3]")
...
class TestZigzagInvariance:
    ...
    def sample_data(self):
        ...
        return pd.DataFrame({"open": open_price, "high": high, "low": low, "close": close})
...
class TestQualityGates:
    ...
    def test_relativity_quality_gates(self, sample_data):
        ...
        X, mask, meta = (
            ...
            np.ones(len(sample_data) - 105 + 1, dtype=bool),
            builder.get_feature_info(),
        )
        ...
        assert (
            X.shape[0] == expected_windows
        ), f"Expected {expected_windows} windows, got {X.shape[0]}"
