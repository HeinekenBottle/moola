...
class TestCausalZigZag:
    ...
    def test_atr_calculation(self, synthetic_ohlc):
        ...
        assert abs(atr - typical_range.mean()) / typical_range.mean() < 2.0, \
    ...
    def test_causality(self, synthetic_ohlc):
        ...
        assert final_state['bar_count'] == len(synthetic_ohlc), \
    ...
    def test_scale_invariance(self, synthetic_ohlc):
        ...
        for i, (s1, s2) in enumerate(zip(swings1, swings2)):
            assert abs(s1 - s2) <= 1, \
                f"Swing {i} timing differs: {s1} vs {s2}"
...
class TestSwingRelative:
    ...
    def test_distance_features(self):
        ...
        assert abs(dist_SH - expected_dist_SH) < 0.01, \
            f"Expected SH distance ~2.5, got {dist_SH}"
        ...
        assert abs(dist_SL - expected_dist_SL) < 0.01, \
            f"Expected SL distance ~2.5, got {dist_SL}"
        
        # Time features should be normalized by K
        expected_bars_SH = 10 / 20  # = 0.5
        expected_bars_SL = 5 / 20    # = 0.25
        
        assert abs(bars_SH_norm - expected_bars_SH) < 0.01, \
            f"Expected bars since SH ~0.5, got {bars_SH_norm}"
        assert abs(bars_SL_norm - expected_bars_SL) < 0.01, \
