...
class FeatureAwareBiLSTMMaskedAutoencoder(nn.Module):
    ...
    def forward(
        self,
        ohlc_masked: torch.Tensor,
        features_masked: torch.Tensor
    ...
    def compute_loss(
        ...
        if ohlc_num_masked == 0:
            ...
            ohlc_recon_loss = F.mse_loss(
                ohlc_reconstruction[ohlc_mask],
                ohlc_original[ohlc_mask],
                reduction='mean'
            )
...
class FeatureAwareMaskingStrategy:
    ...
    def mask_dual_random(
        ...
        feature_num_masked = feature_mask.sum()
        if feature_num_masked > 0:
            features_masked[feature_mask] = feature_mask_token.squeeze(0).expand(feature_num_masked, -1)
