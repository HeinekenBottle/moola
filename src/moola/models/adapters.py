...
class ModuleAdapter:
        ...
        for epoch in range(self.cfg.epochs):
            ...
            for xb, yb, _ptr in train_dl:
                ...
                torch.nn.utils.clip_grad_norm_(
                    self.module.parameters(), self.cfg.max_grad_norm
                )
            ...
            if val_dl is not None:
                ...
                logger.info(
                    f"Epoch {epoch+1}/{self.cfg.epochs} | Train Loss: {avg_train_loss:.4f}"
                )
    ...
    @classmethod
    def load(cls, path: str | Path, *, core_kwargs: Optional[Dict] = None, cfg: Optional[TrainCfg] = None):
        ...
        logger.info(f"Model: {checkpoint.get('model_id', 'unknown')} // {checkpoint.get('codename', 'unknown')}")
