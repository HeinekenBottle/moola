...
class EnhancedOHLCValidator:
    ...
    def _to_dataframe(self, ohlc_data: Union[List[OHLCBar], np.ndarray, pd.DataFrame]) -> Optional[pd.DataFrame]:
        ...
        if isinstance(ohlc_data, pd.DataFrame):
            ...
            if isinstance(ohlc_data[0], OHLCBar):
                data = [(bar.open, bar.high, bar.low, bar.close, bar.timestamp)
    ...
    def _validate_session_continuity(self, df: pd.DataFrame, report: Dict):
        ...
        for i in range(1, len(df)):
            ...
            if time_hours > 48:  # More than 2 days
                price_change = abs(df.iloc[i]['open'] - df.iloc[i-1]['close']) / df.iloc[i-1]['close']
    ...
    def _validate_trend_consistency(self, df: pd.DataFrame, report: Dict):
        ...
        stuck_periods = df['close'].rolling(window=5).apply(
            lambda x: x.nunique() == 1, raw=False
        ).fillna(False)
