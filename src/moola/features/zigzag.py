...
class CausalZigZag:
    ...
    def __init__(
        self, atr_period: int = 10, k: float = 1.2, hybrid_lb: int = 5, hybrid_min_atr: float = 0.5
    ):
    ...
    def _check_hybrid_confirmation(
        self, high: float, low: float, close: float, atr: float, idx: int
    ) -> bool:
        """Check hybrid confirmation rule."""
        if (
            self.extreme_candidate_idx is None
            or atr == 0
            or idx - self.extreme_candidate_idx > self.hybrid_lb
        ):
            return False
    ...
    def update(
    ...
    ) -> tuple[Optional[tuple[int, Optional[float]]], Optional[tuple[int, Optional[float]]]]:
        ...
        if self.current_trend == "up" and (
            self.extreme_candidate_price is None or high > self.extreme_candidate_price
        ):
            self.extreme_candidate_price = high
            self.extreme_candidate_idx = idx
        elif self.current_trend == "down" and (
            self.extreme_candidate_price is None or low < self.extreme_candidate_price
        ):
    ...
    def get_state(self) -> dict:
        ...
        return {
            ...
            "prev_SH": (
                (self.prev_SH_idx, self.prev_SH_price) if self.prev_SH_idx is not None else None
            ),
            "prev_SL": (
                (self.prev_SL_idx, self.prev_SL_price) if self.prev_SL_idx is not None else None
            ),
...
def swing_relative(
    close: float,
    prev_SH_price: Optional[float],
    prev_SL_price: Optional[float],
    atr: float,
    bars_since_SH: int,
    bars_since_SL: int,
    K: float,
...
def detect_zigzag_pivots(
    prices: np.ndarray, k: float, min_segments: int, max_segments: int
