...
class RelativityConfig(BaseModel):
    ...
    zigzag_hybrid_min_retrace_atr: float = Field(0.5, description="Minimum retrace for hybrid confirmation")
...
def candle_shape(open_price: float, high: float, low: float, close: float, 
                 ema_range: float, eps: float = 1e-6) -> Tuple[float, float, float, float, float, float]:
...
def build_features(df: pd.DataFrame, cfg: RelativityConfig) -> Tuple[np.ndarray, np.ndarray, Dict]:
    ...
    """
    # Validate required columns (OHLC only)
    ...
    X = np.zeros((n_windows, window_length, 10), dtype=np.float32)
...
def build_relativity_features(data: pd.DataFrame, config: Optional[Dict] = None) -> Tuple[np.ndarray, np.ndarray, Dict]:
    """Build relativity features with output contract.
    
    AGENTS.md Section 15: Every feature builder returns X, mask, meta.
    
    Args:
        data: OHLC DataFrame (no volume)
