...
from ..features.relativity import RelativityConfig, build_relativity_features
...
class WindowedConfig(BaseModel):
    ...
    jitter_sigma: float = Field(
        0.03, description="Gaussian jitter sigma for augmentation (PDF: 0.03)"
    )
    feature_config: Optional[dict[str, Any]] = Field(None, description="Relativity feature config")
    splits: Optional[dict[str, str]] = Field(
        None, description="Date-based splits (train_end, val_end, test_end)"
    )
...
class WindowedDataset(Dataset):
    ...
    def __init__(
        self, df: pd.DataFrame, config: WindowedConfig, feature_cache: Optional[np.ndarray] = None
    ):
        ...
        if feature_cache is None:
            ...
            self.X_full, self.valid_mask_full, _ = build_relativity_features(
                self.df, relativity_cfg.dict()
            )
        else:
    ...
    def _apply_padding(
        self, X: np.ndarray, valid_mask: np.ndarray, window_start: int
...
def save_split_manifest(
    ...
    config: WindowedConfig,
    save_path: str = "artifacts/jade_pretrain/splits.json",
) -> dict:
...
def main():
    ...
    try:
        ...
        # Save manifest
        save_split_manifest(train_df, val_df, test_df, config, f"{args.output_dir}/splits.json")
