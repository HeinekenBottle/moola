...
class Feature11DIntegrator:
    ...
    def create_dual_input_dataset(
        self, data_path: Path, save_path: Optional[Path] = None
    ) -> pd.DataFrame:
        ...
        for i in range(N):
            ...
            enhanced_features = np.concatenate(
                [ohlc_data[i], relative_11d_data[i]], axis=1  # [105, 4] OHLC  # [105, 11] relative
            )  # [105, 15]
    ...
    def validate_feature_quality(
        self, ohlc_data: np.ndarray, relative_data: np.ndarray
    ) -> Dict[str, Any]:
        ...
        """
        report = {"ohlc_quality": {}, "relative_quality": {}, "cross_validation": {}}
    ...
    def get_feature_importance_analysis(
        self, relative_data: np.ndarray, labels: Optional[np.ndarray] = None
    ) -> Dict[str, Any]:
        ...
        analysis = {"feature_variance": {}, "feature_correlation": {}, "temporal_stability": {}}
        ...
        for i, name in enumerate(feature_names):
            ...
            for t in range(1, T):
                corr = np.corrcoef(
                    relative_data[:, t - 1, i].flatten(), relative_data[:, t, i].flatten()
                )[0, 1]
            ...
            analysis["temporal_stability"][name] = {
                "mean_autocorrelation": (
                    float(np.mean(temporal_autocorr)) if temporal_autocorr else 0.0
                ),
