...
class FinancialDataValidator:
    ...
    def _validate_statistical_properties(self, original_data: np.ndarray,
                                       generated_data: np.ndarray) -> Dict[str, float]:
    ...
    def _validate_temporal_dependencies(self, original_data: np.ndarray,
                                     generated_data: np.ndarray) -> Dict[str, float]:
    ...
    def _validate_market_realism(self, original_data: np.ndarray,
                               generated_data: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            if orig_spreads and gen_spreads:
                ...
                similarity = 1.0 - abs(gen_mean_spread - orig_mean_spread) / (orig_mean_spread + 1e-8)
            ...
            for sample in original_data:
                gaps = (sample[1:, 0] - sample[:-1, 3]) / sample[:-1, 3]  # (Open-Prev Close)/Prev Close
    ...
    def _validate_pattern_similarity(self, original_data: np.ndarray,
                                  generated_data: np.ndarray) -> Dict[str, float]:
    ...
    def _validate_risk_metrics(self, original_data: np.ndarray,
                            generated_data: np.ndarray) -> Dict[str, float]:
    ...
    def validate_pseudo_samples(self, original_data: np.ndarray,
                              generated_data: np.ndarray) -> ValidationReport:
        ...
        statistical_similarity = self._validate_statistical_properties(original_data, generated_data)
        ...
        valid_metrics = {k: v for k, v in all_metrics.items() if isinstance(v, (int, float)) and not np.isnan(v)}
    ...
    def _generate_recommendations(self, metrics: Dict[str, float],
        ...
        if metrics.get('volatility_clustering_similarity', 1.0) < thresholds['volatility_clustering']:
            recommendations.append(
    ...
    def _check_thresholds(self, metrics: Dict[str, float],
        ...
        valid_metrics = {k: v for k, v in metrics.items() if isinstance(v, (int, float)) and not np.isnan(v)}
        if valid_metrics:
...
class QualityMetricsVisualizer:
    ...
    def plot_validation_results(self, report: ValidationReport,
        ...
        axes[0, 0].bar(['Overall Quality'], [report.overall_quality_score],
        ...
        # OHLC integrity
        axes[0, 1].bar(['OHLC Integrity'], [report.ohlc_integrity],
        ...
        # Statistical similarity
        stat_metrics = {k: v for k, v in report.statistical_similarity.items()
                       if isinstance(v, (int, float))}
        if stat_metrics:
            axes[0, 2].barh(list(stat_metrics.keys()), list(stat_metrics.values()),
                           color=self.colors[1])
        ...
        # Temporal consistency
        temp_metrics = {k: v for k, v in report.temporal_consistency.items()
                       if isinstance(v, (int, float))}
        if temp_metrics:
            axes[1, 0].barh(list(temp_metrics.keys()), list(temp_metrics.values()),
                           color=self.colors[2])
        ...
        # Market realism
        market_metrics = {k: v for k, v in report.market_realism.items()
                         if isinstance(v, (int, float))}
        if market_metrics:
            axes[1, 1].barh(list(market_metrics.keys()), list(market_metrics.values()),
                           color=self.colors[3])
        ...
        # Risk metrics
        risk_metrics = {k: v for k, v in report.risk_metrics.items()
                       if isinstance(v, (int, float))}
        if risk_metrics:
            axes[1, 2].barh(list(risk_metrics.keys()), list(risk_metrics.values()),
                           color=self.colors[4])
    ...
    def plot_distribution_comparison(self, original_data: np.ndarray,
        ...
        """
        fig, axes = plt.subplots(2, 3, figsize=(18, 12))
        fig.suptitle('Distribution Comparison: Original vs Generated', fontsize=16, fontweight='bold')
        ...
        orig_returns = orig_returns[(orig_returns > np.percentile(orig_returns, 1)) &
                                   (orig_returns < np.percentile(orig_returns, 99))]
        gen_returns = gen_returns[(gen_returns > np.percentile(gen_returns, 1)) &
                                 (gen_returns < np.percentile(gen_returns, 99))]

        # Returns distribution
        axes[0, 0].hist(orig_returns, bins=50, alpha=0.7, label='Original', density=True, color=self.colors[0])
        axes[0, 0].hist(gen_returns, bins=50, alpha=0.7, label='Generated', density=True, color=self.colors[1])
        ...
        axes[1, 0].plot(lags[:len(gen_autocorr)], gen_autocorr, 's-', label='Generated', color=self.colors[1])
        ...
        axes[1, 0].hist(orig_abs_returns, bins=30, alpha=0.7, label='Original', density=True, color=self.colors[0])
        axes[1, 0].hist(gen_abs_returns, bins=30, alpha=0.7, label='Generated', density=True, color=self.colors[1])
        ...
        axes[1, 1].hist(orig_prices, bins=50, alpha=0.7, label='Original', density=True, color=self.colors[0])
        axes[1, 1].hist(gen_prices, bins=50, alpha=0.7, label='Generated', density=True, color=self.colors[1])
        ...
        axes[1, 2].plot(timesteps, original_data[sample_idx, :, 3], label='Original', color=self.colors[0])
        axes[1, 2].plot(timesteps, generated_data[sample_idx, :, 3], label='Generated', color=self.colors[1])
