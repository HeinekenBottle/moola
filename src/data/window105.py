...
class Window105Dataset:
    ...
    def _load_training_data(self) -> None:
        ...
        train_path = self.data_root / "processed" / "train.parquet"
        ...
        required_cols = ['window_id', 'label', 'expansion_start', 'expansion_end', 'features']
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        window_data = {
            ...
            'label': sample['label'],
            'expansion_start': int(sample['expansion_start']),
            'expansion_end': int(sample['expansion_end']),
            'ohlc': ohlc_data,
            'features': sample['features'],  # Raw features array (105 bars × 4 OHLC)
            'center_timestamp': self._extract_timestamp(sample['window_id']),
        }
    ...
    def _get_ohlc_data(self, window_id: str) -> np.ndarray:
        ...
        # Features should be (105, 4) numpy array with OHLC data
    ...
    def _extract_timestamp(self, window_id: str) -> str:
        """
        Extract/estimate timestamp from window_id.
        ...
        """
        # Window IDs are like "104_exp_1", "197_exp_1" etc.
        # Extract the base number and use as pseudo-timestamp
        try:
            base_num = int(window_id.split('_')[0])
            # Convert to a reasonable datetime format
            # Using days since epoch as a simple approach
            import datetime
            base_date = datetime.datetime(2020, 1, 1)  # Arbitrary base date
            timestamp = base_date + datetime.timedelta(days=base_num)
