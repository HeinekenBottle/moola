...
class Window105Dataset:
    ...
    def _load_training_data(self) -> None:
        ...
        logger.info(
            f"Loaded {len(self.samples)} training samples from batch_204_annotation_ready.parquet"
        )
    ...
    def _load_ohlc_features(self) -> None:
        ...
        if not features_path.exists():
            logger.warning(
                f"OHLC features not found at {features_path}, will generate from training data"
            )
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        window_data = {
            ...
            "expansion_start": (
                int(sample["expansion_start"]) if "expansion_start" in sample else None
            ),
    ...
    def _get_ohlc_data(self, window_id: str) -> np.ndarray:
        ...
        if self.ohlc_features is not None:
            ...
            window_ohlc = self.ohlc_features[
                self.ohlc_features["window_id"] == window_id
            ].sort_values("bar_index")
            if len(window_ohlc) == 105:
                return window_ohlc[["open", "high", "low", "close"]].values
            else:
                logger.warning(
                    f"Found {len(window_ohlc)} bars for window {window_id}, expected 105"
                )
    ...
    def get_summary_stats(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "expansion_stats": {
                ...
                "avg_length": float(
                    (self.samples["expansion_end"] - self.samples["expansion_start"]).mean()
                ),
            },
            "priority_windows": (
                len(self.priority_samples) if not self.priority_samples.empty else 0
            ),
        }
...
def test_integration():
    ...
    try:
        ...
        for i in range(min(3, len(dataset))):
            sample = dataset[i]
            print(
                f"✓ Sample {i}: {sample['window_id']} ({sample['label']}) - OHLC shape: {sample['ohlc'].shape}"
            )
